...
export const GalleryDetailPage = ({ selectedTopic, TOPICS, WORDS_BY_TOPIC, setCurrentPage, learnedCount, topicTotal, topicProgress, startQuiz, onStartTTS, toggleBookmark, isBookmarked, learnedWords }) => {
  ...
  return (
    <div className="bg-[#F8F9FF] min-h-screen py-6 md:py-12 px-4 md:px-6 lg:px-24">
      <div className="max-w-7xl mx-auto space-y-6 md:space-y-10">
         ...
         <div className="flex flex-col lg:flex-row justify-between items-start gap-6 md:gap-8">
            <div className="flex-1 space-y-3">
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-gray-900 tracking-tight">{topic?.title}</h1>
              <p className="text-base md:text-lg text-gray-500 max-w-2xl font-medium leading-relaxed">{topic?.desc}</p>
            </div>
         ...
         <div className="flex flex-col lg:flex-row gap-6">
            <div className="w-full lg:w-2/3 h-48 md:h-64 lg:h-80 rounded-[24px] md:rounded-3xl overflow-hidden relative shadow-sm">
               ...
               <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-6 md:p-8 text-white">
                 ...
                 <h3 className="text-xl md:text-2xl lg:text-3xl font-black">{topic?.title}</h3>
               </div>
            </div>
            <div className="w-full lg:w-1/3 h-48 md:h-64 lg:h-80 bg-[#006D5B] rounded-[24px] md:rounded-3xl p-6 md:p-8 text-white shadow-sm flex flex-col justify-center relative overflow-hidden">
               <Zap className="w-6 h-6 md:w-8 md:h-8 mb-4 text-emerald-300" />
               <h4 className="text-lg md:text-xl font-black mb-3">Precision over Prolixity</h4>
               <p className="text-emerald-50 text-xs md:text-sm leading-relaxed opacity-90">In academic discourse, clarity is paramount. Every word must serve a specific diagnostic or analytical purpose to ensure replicability and peer understanding.</p>
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {(WORDS_BY_TOPIC[selectedTopic] || []).map((w, idx) => (
              <div key={idx} className="bg-white p-6 md:p-8 rounded-[24px] md:rounded-3xl shadow-sm border border-gray-100 flex flex-col hover:shadow-2xl transition-all duration-500 ease-out hover:-translate-y-2 group">
                 ...
                 <div className="space-y-3 mb-8 flex-1">
                    <h3 className="text-xl md:text-2xl font-black text-[#006D5B] tracking-tight">{w.term}</h3>
